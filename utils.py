...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        ...
        if is_batchnorm:
            self.conv1 = nn.Sequential(nn.Conv2d(int(in_size), int(out_size), kernel_size=3, stride=1, padding=1),
                                       nn.BatchNorm2d(int(out_size)),
                                       nn.ReLU(),
                                       )

            self.conv2 = nn.Sequential(nn.Conv2d(int(out_size), int(out_size), kernel_size=3, stride=1, padding=1),
                                       nn.BatchNorm2d(int(out_size)),
                                       nn.ReLU(),
                                      )
...
class unetDown_new(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        super(unetDown_new, self).__init__()
        self.conv = unetConv2(in_size, out_size, is_batchnorm)
        self.down = nn.Conv2d(out_size, out_size, kernel_size=3, stride=2, padding=1)
...
class unetDown(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        ...
        #print("Down: ", "in_size: ",in_size," out_size: ",out_size)
        self.conv = unetConv2(in_size, out_size, is_batchnorm)
        self.down = nn.MaxPool2d(2, 2)

    def forward(self, inputs):
        outputs1 = self.conv(inputs)
        #print("Down: ",outputs1.shape)
        outputs2 = self.down(outputs1)
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv):
        ...
        if is_deconv:
            self.up = nn.Sequential(nn.ConvTranspose2d(int(in_size), int(out_size), kernel_size=3,stride=2,padding=1, output_padding=1),)
        else:
    ...
    def forward(self, inputs1, inputs2):
        ...
        #print("inputs2: ",inputs2.size(),"outputs2",outputs2.size(),"inputs1: ",inputs1.size(),offset)
        ...
        #print "padding",padding,outputs1.size(),outputs2.size()
